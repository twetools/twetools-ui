...
interface ModalAlertConfirmProps {
  isOpen: boolean;
  onClose: () => void;
  title: string;
  message: string;
...
export default function ModalAlertConfirm({
  isOpen,
  onClose,
  title,
  message,
    ...
    switch (variant) {
      ...
      default: // warning
        return {
          ...
          iconBackgroundClassName: "fill-warning-50 dark:fill-warning-500/15",
          buttonClassName: "bg-warning-500 hover:bg-warning-600",
        };
    }
  };

  const { icon, iconBackgroundClassName, buttonClassName } = getVariantConfig();
  ...
  return (
    ...
    >
      <div className="text-center">
        {/* Icon background - using same pattern as base ModalAlert */}
        <div className="relative flex items-center justify-center z-1 mb-7">
          <svg
            className={iconBackgroundClassName}
            width="90"
            height="90"
            viewBox="0 0 90 90"
            fill="none"
        ...
        {/* Title and message - using exact same classes as base ModalAlert */}
        ...
        {/* Buttons - using same layout pattern as base ModalAlert */}
        <div className="flex items-center justify-center w-full gap-3 mt-7">
          <button
            type="button"
            onClick={handleCancel}
            className="flex justify-center w-full px-4 py-3 text-sm font-medium text-gray-900 bg-white border border-gray-300 rounded-lg shadow-theme-xs hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:hover:bg-gray-700 sm:w-auto"
          >
            {cancelText}
          </button>
          <button
            ...
            className={`flex justify-center w-full px-4 py-3 text-sm font-medium text-white rounded-lg shadow-theme-xs sm:w-auto ${buttonClassName}`}
          >
            {confirmText}
          </button>
        </div>
